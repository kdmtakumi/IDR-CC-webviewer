...
if (verbose)  cout << "\n SetUpConnections "  << endl;
 if (seqNb == 1)   SetUpConnections();
if (verbose)  cout << "\n SetUpScaledParameters "  << endl;
...
if (verbose)  cout << "\n FB computation  kStates = " <<  kStates  << endl;
...
if (nb < 0.0)  nb = - nb; 
if (nb > 00001)  cout  << "\n Failed ForwBackw-PrecisionCheck" << endl;  	
...
for (pos=0; pos<=totalNumber+1; pos++)
     ...
     nb = probSeq - 1.0;
     if (nb < 0.0)  nb = - nb; 
     if (nb > 0.00001)   cout  << "\n Failed probSeq=1 PrecisionCheck" << endl;  	
     }
if (writeControls || FBdebugging)     fclose( cFpWrite);
if (verbose)  cout << "\n FB computation concluded"  << endl;
}
/* FBalgorithm
************************************/
// ---------------------------------------------------------------------------
