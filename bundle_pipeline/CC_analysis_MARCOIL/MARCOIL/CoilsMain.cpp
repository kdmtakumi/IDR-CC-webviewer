...
if (verbose) cout << " CoilsMain, matrixType = " << matrixType << endl;
if (verbose)   cout << " CoilsMain, matrixType = " << matrixType << endl;
...
if  (matrixType<3) 
	...
	if (verbose)  cout << "finished ReadScoresCoils" << " using the file " << scoreFile << endl;
...
if (verbose)    cout << "Coils file completed"  << endl;
...
for (state=0; state < 8; state++)
     ...
     for (aa=1; aa <=20 ; aa++)
	     ...
	     if ( end || (fscanf(cFrPar, "%d\n ",&number) == EOF)) 
		     {emProb[state][aa] = 0; end = true; cout << "\n end" ;}
