...
# -*- coding: utf-8 -*-
"""
IDRとCCが両方とも30%以上あるタンパク質を抽出
"""
import csv

def extract_high_idr_cc():
    """
    IDR_Percentage >= 30% かつ CC_Percentage >= 30% のタンパク質を抽出
    """
    input_file = "../IDR+CC_DB/all_human_protein_database_with_IDR-CCinformation.csv"
    output_file = "../proteins_with_high_IDR_and_CC.csv"

    print(f"ファイル読み込み中: {input_file}")

    high_idr_cc_proteins = []
    total_count = 0
    ...
    with open(input_file, 'r', encoding='utf-8') as f:
        ...
        for row in reader:
            ...
            try:
                ...
                if idr_pct >= 30.0 and cc_pct >= 30.0:
                    high_idr_cc_proteins.append(row)
    ...
    with open(output_file, 'w', newline='', encoding='utf-8') as f:
        ...
        writer.writeheader()
        writer.writerows(high_idr_cc_proteins)

    # 統計
    print(f"\n{'='*70}")
    print(f"総タンパク質数: {total_count:,}")
    print(f"抽出件数: {len(high_idr_cc_proteins):,} ({len(high_idr_cc_proteins)/total_count*100:.2f}%)")
    print(f"\n条件:")
    print(f"  IDR_Percentage >= 30%")
    print(f"  AND")
    print(f"  CC_Percentage >= 30%")
    print(f"\n完了: {output_file} に保存")
    print(f"{'='*70}")

    # 上位10件を表示
    if high_idr_cc_proteins:
        # IDR+CC合計が高い順にソート
