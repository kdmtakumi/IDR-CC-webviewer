...
# -*- coding: utf-8 -*-
"""
指定範囲のタンパク質にSubcellular Location情報を追加
"""
import requests
import csv
import time
...
def add_subcellular_location_chunk(start_idx, end_idx):
    """
    指定範囲のタンパク質にSubcellular Location列を追加
    """
...
if __name__ == "__main__":
    if len(sys.argv) != 3:
        print("Usage: python add_subcellular_location_chunk.py <start_idx> <end_idx>")
        sys.exit(1)

    start_idx = int(sys.argv[1])
    end_idx = int(sys.argv[2])
    add_subcellular_location_chunk(start_idx, end_idx)
